...
from numpy import arctan2, sqrt, sin, cos, tan, asarray, clip, mean, pi, sign
        ...
        while not exit:
            ...
            for point in allPoints:
                for near in point.near:
                    ...
                    #fScale = (point.distTo(near) + l0) / (2*l0)
                    dt = 0.2
                    fl.append(abs(f * dt))
                    near.repulsiveForce(f * dt, point)
...
if __name__ == '__main__':
    ...
    corpus.unispring(0.01, 0.02, exportPeriod=1, client=None, limit=200*(len(vertices)/4))