...
class Corpus():
    ...
    def uniform(self, k, client=None, store=True):
    ...
    def unispring(
            self, k, uniform=False, exportPeriod=0, client=None,
...
if __name__ == '__main__':
        ...
        def exportToMax(self, client=None):
            ...
            for buffer in self.buffers:
                for p in buffer.points:
                    for pt in p.near:
                        self.ax.plot([p.x,pt.x],[p.y,pt.y],'--',color='grey')
                    self.ax.arrow(p.x, p.y, p.pushX, p.pushY, color='red', lw=1)
                    self.ax.plot(p.x,p.y,'.',color='orange')
            plt.pause(0.05)
    ...
    with plt.ion():
        corpus.unispring(0.8, exportPeriod=1, client=None, limit=500)
    corpus.exportToMax()
    plt.show()